...
export default function App() {
  ...
  return (
    <div className={`min-h-screen w-full lg:h-screen lg:w-screen lg:overflow-hidden font-sans flex flex-col transition-colors duration-300 selection:bg-indigo-500 selection:text-white relative ${  
      darkMode 
    ...
    >
      ...
      {/* HEADER */}
      <header className={`h-12 px-3 sm:px-5 flex-shrink-0 flex justify-between items-center z-40 border-b transition-colors duration-300 relative ${  
        darkMode 
        ...
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-xl bg-gradient-to-tr from-rose-400 via-teal-500 to-indigo-600 flex items-center justify-center text-white shadow-md shadow-rose-500/20">
            <Leaf size={14} className="text-white animate-pulse" />  
          </div>
          <div>
            <h1 className={`text-xs sm:text-sm md:text-base font-black tracking-tight bg-gradient-to-r ${  
        ...
        <div className="absolute left-1/2 transform -translate-x-1/2 hidden md:flex items-center">
          <div className={`px-4 py-1 rounded-full border shadow-inner transition-all duration-300 ${  
            darkMode 
              ? 'bg-slate-900/40 border-amber-500/30 shadow-amber-500/10'  
              : 'bg-white/50 border-amber-400/40 shadow-amber-500/10'  
          }`}>
            <span className={`text-sm sm:text-base font-black tracking-wider uppercase bg-gradient-to-r ${  
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <div className={`hidden sm:block text-[11px] font-bold font-serif px-3 py-1 rounded-xl border shadow-sm ${  
          ...
          <button 
            ...
            className={`p-1.5 sm:p-2 rounded-xl border transition-all shadow-sm flex items-center justify-center ${  
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-y-auto lg:overflow-hidden relative">
        ...
        {/* SIDEBAR */}
        <aside className={`w-full lg:w-48 flex-shrink-0 border-b lg:border-r flex flex-row lg:flex-col justify-between items-center lg:items-stretch p-2 gap-2 overflow-x-auto ${  
          darkMode ? 'bg-[#181e22]/80 border-slate-800' : 'bg-white/60 border-rose-100/60 backdrop-blur-md'  
        }`}>
          <div className="flex flex-row lg:flex-col gap-2 w-full overflow-x-auto lg:overflow-visible items-center lg:items-stretch">
            <button 
              onClick={() => setActiveTab('dashboard')}  
              className={`flex items-center gap-2.5 px-3 py-2.5 rounded-xl font-extrabold text-xs transition-all whitespace-nowrap lg:w-full text-left shadow-sm ${  
                activeTab === 'dashboard' 
    ...
    <div className="space-y-1 animate-pulse">
      <h2 className="text-base md:text-lg lg:text-xl font-extrabold tracking-wide leading-relaxed">
        ...
        <div className="mt-1">
          ...
          <div className="pt-0 lg:pt-2 lg:border-t border-rose-200/40 dark:border-slate-800 flex-shrink-0">
            <button 
              ...
              title="Logout"
              className="w-full flex items-center justify-center lg:justify-start gap-2.5 px-3 py-2 rounded-xl font-bold text-xs bg-rose-500/10 text-rose-600 dark:text-rose-400 hover:bg-rose-500 hover:text-white transition-all duration-300 group whitespace-nowrap"
            >
              <LogOut size={15} className="transition-transform duration-300 group-hover:-translate-x-1 flex-shrink-0" />  
        ...
        {isLogoutConfirmOpen && (
          ...
          >
            <div 
              className={`border p-5 rounded-3xl w-full max-w-sm relative flex flex-col gap-4 shadow-2xl ${  
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-black text-sm">Are you sure?</h3>  
                  <p className="text-[11px] text-slate-400 font-semibold">Do you really want to log out?</p>  
                </div>
              </div>
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button 
                  ...
                  className="flex-1 py-2 rounded-xl font-black text-xs bg-gradient-to-r from-rose-500 to-teal-500 text-white shadow-md shadow-rose-500/25 hover:opacity-95 transition"
                >
                  Yes, Logout
                </button>
              </div>
        ...
        {isSlipModalOpen && (
          ...
          >
            <div 
              className="bg-white text-slate-900 border border-slate-200 p-5 rounded-3xl w-full max-w-md relative flex flex-col gap-4 shadow-2xl my-auto"
              onClick={(e) => e.stopPropagation()}  
            >
              <button onClick={() => setIsSlipModalOpen(false)} className="absolute top-3 right-3 p-1.5 rounded-xl text-slate-400 hover:text-rose-500 transition">
                <X size={18}/>  
              </button>

              <div ref={slipRef} className="bg-white border-2 border-dashed border-rose-300 p-4 rounded-2xl flex flex-col gap-3 text-slate-900">
                <div className="text-center border-b-2 border-rose-500 pb-2">
                  <h2 className="text-lg font-black tracking-tight text-rose-700 uppercase">
                    Hakeem Amjad Maqsood
                  </h2>
                  <p className="text-[11px] font-extrabold text-slate-600">M Mehmood Unani Dawakhana</p>  
                  <p className="text-[10px] font-semibold text-slate-500">Gulshan-e-Iqbal, Karachi | Ph: +92 300 7071814</p>  
                </div>

                <div className="flex justify-between items-center text-[11px] font-bold text-slate-500 bg-rose-50 px-2.5 py-1 rounded-lg border border-rose-100">
                ...
                <div className="space-y-2 py-0.5">
                  <div className="flex justify-between items-center border-b border-slate-100 pb-1">
                    ...
                    <span className="text-xs font-black text-slate-900">{selectedPatient ? selectedPatient.Name : (newPatient.Name || "-")}</span>  
                  </div>

                  {(selectedPatient?.Address || newPatient.Address) && (
                    <div className="flex justify-between items-center border-b border-slate-100 pb-1">
                      <span className="text-[11px] font-bold text-slate-400">Address:</span>
                      <span className="text-xs font-bold text-slate-700">{selectedPatient ? selectedPatient.Address : newPatient.Address}</span>  
                    </div>
                  )}

                  {(selectedPatient?.Additional || newPatient.Additional) && (
                    <div className="flex justify-between items-center border-b border-slate-100 pb-1">
                      <span className="text-[11px] font-bold text-slate-400">Additional Info:</span>
                      <span className="text-xs font-bold text-slate-700">{selectedPatient ? selectedPatient.Additional : newPatient.Additional}</span>  
                    </div>
                  )}

                  <div className="flex flex-col gap-0.5 pt-0.5 border-b border-slate-100 pb-1.5">
                    <span className="text-[11px] font-bold text-slate-400">Prescribed Medicine / Syrup:</span>
                    <span className="text-xs font-extrabold text-teal-800 bg-teal-50 p-2 rounded-xl border border-teal-100">
                  ...
                  <div className="flex justify-between items-center pt-1">
                    ...
                    <span className="text-sm font-black text-rose-600">Rs. {selectedPatient ? selectedPatient.Amount : (newPatient.Amount || "0")}</span>  
                  </div>
                </div>

                <div className="text-center pt-2 border-t border-slate-200">
                  <p className="text-[10px] font-bold text-slate-400 italic">Get well soon! Thank you for visiting.</p>
                </div>
              </div>
              ...
              <div className="flex gap-2 pt-1">
                <button 
                  ...
                  className="flex-1 bg-gradient-to-r from-rose-500 to-teal-500 hover:opacity-95 text-white py-2.5 px-3 rounded-xl font-black text-xs shadow-md shadow-rose-500/25 flex items-center justify-center gap-1.5 transition"
                >
                  <Download size={14} /> Download PDF Slip
                </button>
                <button 
                  onClick={() => setIsSlipModalOpen(false)}
                  className="bg-slate-100 hover:bg-slate-200 text-slate-700 py-2.5 px-3 rounded-xl font-bold text-xs transition"
                >
                  Close
                </button>
              </div>
        ...
        {/* MAIN VIEWPORT AREA */}
        <main className="flex-1 overflow-y-auto lg:overflow-hidden p-2.5 sm:p-3 flex flex-col lg:flex-row gap-3">

          {/* POPUP MODAL FOR EDITING FROM ALL PATIENTS */}
          {isModalOpen && (
            <div 
              onClick={(e) => { if (e.target === e.currentTarget) setIsModalOpen(false); }}  
              className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm z-50 flex items-center justify-center p-3"
            >
              <div 
                className={`border p-4 rounded-3xl w-full max-w-lg relative flex flex-col shadow-2xl max-h-[90vh] overflow-y-auto ${  
                  darkMode ? 'bg-[#181e22] border-slate-800 text-slate-100' : 'bg-white border-rose-100 text-slate-900 shadow-rose-500/10'  
                }`} 
                ref={modalDropdownRef}  
                onClick={(e) => e.stopPropagation()}  
              >
                <button onClick={() => setIsModalOpen(false)} className="absolute top-3 right-3 p-1.5 rounded-xl text-slate-400 hover:text-rose-500 transition">
                  <X size={16}/>  
                </button>
                
                <h3 className="font-black mb-2.5 text-xs sm:text-sm flex items-center gap-2">
                  <Activity size={16} className="text-emerald-500 animate-pulse" />  
                  {modalPatient.Id ? "Edit Patient Details" : "Add New Patient"}  
                </h3>

                <div className={`border rounded-2xl p-3 mb-3 flex flex-col gap-2 shadow-sm ${  
                  darkMode ? 'bg-slate-900/80 border-slate-700' : 'bg-rose-50/60 border-rose-200'  
                }`}>
                  <div className="flex justify-between items-center border-b pb-1.5 border-rose-200/50 dark:border-slate-800">
                    <h4 className="font-extrabold text-xs sm:text-sm flex items-center gap-1.5 text-rose-600 dark:text-rose-400">
                      <HeartPulse size={15} className="text-rose-500 animate-pulse"/> Profile Summary  
                    </h4>
                    <span className="text-[11px] font-bold text-slate-400">{formatDateToDisplay(modalPatient.Date || getTodayDateInput())}</span>  
                  </div>

                  <div className="flex justify-between items-center text-xs sm:text-sm">
                    <span className="font-black truncate pr-2 text-sm">{modalPatient.Name || "Patient Name"}</span>  
                    <span className="font-black text-rose-500 text-sm whitespace-nowrap">{modalPatient.Amount || "0"}</span>  
                  </div>
...
<div className="text-[11px] font-black text-slate-700 dark:text-slate-200 truncate">
  ...
  <span className="font-bold text-slate-400">Additional: </span>{modalPatient.Additional}  
                  ...
                  <div className="text-[11px] font-bold text-rose-500/90 leading-relaxed">
                ...
                <div className="space-y-2.5">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <div className="flex flex-col gap-0.5">
                      <label className="text-[10px] font-bold text-slate-400">Patient Name</label>
                      <input placeholder="Enter Name" className={`px-3 py-2 rounded-xl border text-xs font-bold outline-none transition h-9 ${darkMode ? 'bg-slate-900 border-slate-800 text-white focus:border-rose-500' : 'bg-rose-50/30 border-rose-100 focus:border-rose-400'}`} value={modalPatient.Name} onChange={(e) => setModalPatient({...modalPatient, Name: e.target.value})} />  
                    </div>
                    <div className="flex flex-col gap-0.5">
                      <label className="text-[10px] font-bold text-slate-400">Date</label>
                      <input type="date" className={`px-3 py-2 rounded-xl border text-xs font-bold outline-none transition h-9 ${darkMode ? 'bg-slate-900 border-slate-800 text-white focus:border-rose-500' : 'bg-rose-50/30 border-rose-100 focus:border-rose-400'}`} value={modalPatient.Date} onChange={(e) => setModalPatient({...modalPatient, Date: e.target.value})} />  
                    </div>
                    <div className="flex flex-col gap-0.5">
                      <label className="text-[10px] font-bold text-slate-400">Address</label>
                      <input placeholder="Enter Address" className={`px-3 py-2 rounded-xl border text-xs font-semibold outline-none transition h-9 ${darkMode ? 'bg-slate-900 border-slate-800 text-white focus:border-rose-500' : 'bg-rose-50/30 border-rose-100 focus:border-rose-400'}`} value={modalPatient.Address} onChange={(e) => setModalPatient({...modalPatient, Address: e.target.value})} />  
                    </div>
                    <div className="flex flex-col gap-0.5">
                      <label className="text-[10px] font-bold text-slate-400">Additional</label>
                      <input placeholder="Additional info" className={`px-3 py-2 rounded-xl border text-xs font-semibold outline-none transition h-9 ${darkMode ? 'bg-slate-900 border-slate-800 text-white focus:border-rose-500' : 'bg-rose-50/30 border-rose-100 focus:border-rose-400'}`} value={modalPatient.Additional} onChange={(e) => setModalPatient({...modalPatient, Additional: e.target.value})} />  
                    </div>
                  </div>

                  <div className={`grid grid-cols-1 sm:grid-cols-3 gap-2 p-2.5 border rounded-2xl ${darkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-rose-50/20 border-rose-100'}`}>
                    {dropdownData.map((drop, idx) => {  
                      ...
                      return (
                        <div key={idx} className="flex flex-col gap-0.5 relative">
                          ...
                          <div onClick={(e) => { e.stopPropagation(); setOpenModalDropdown(isOpen ? null : drop.name); }} className={`px-2.5 py-1.5 rounded-xl border text-[11px] font-bold flex justify-between items-center cursor-pointer transition h-9 ${darkMode ? 'bg-slate-900 border-slate-800 text-slate-200' : 'bg-white border-rose-100 text-slate-800 hover:border-rose-300'}`}>
                            <span className="truncate">{selectedItems.length > 0 ? selectedItems.join(", ") : `Select...`}</span>
                            <ChevronDown size={13} className="text-rose-500 flex-shrink-0" />  
                          </div>
                          {isOpen && (
                            <div className={`absolute bottom-[calc(100%+2px)] left-0 right-0 mb-1 border rounded-xl shadow-xl z-30 max-h-36 overflow-y-auto p-1 space-y-1 ${darkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-rose-100'}`}>
                              {drop.options.map((opt, optIdx) => {
                                ...
                                return (
                                  <div 
                                    ...
                                    className={`p-1.5 text-xs font-bold rounded-lg cursor-pointer flex justify-between items-center transition ${isSelected ? 'bg-rose-500 text-white' : darkMode ? 'hover:bg-slate-800 text-slate-300' : 'hover:bg-rose-50 text-slate-800'}`}
                                  >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 items-center">
                    <label 
                      className={`flex items-center gap-2.5 px-3 rounded-xl font-bold text-xs cursor-pointer border h-9 transition select-none ${  
                    ...
                    >
                      <div className={`w-4 h-4 rounded-md border flex items-center justify-center transition-colors ${modalPatient.isCrushed ? 'bg-emerald-500 border-emerald-500 text-white' : darkMode ? 'border-slate-700 bg-slate-800' : 'border-rose-200 bg-white'}`}>
                        {modalPatient.isCrushed && <span className="text-[10px] font-black leading-none">✓</span>}  
                      </div>
                ...
                <div className="pt-3 border-t mt-3 flex gap-2 flex-shrink-0 border-rose-100/60">
                  {modalPatient.Id ? (
                    <>
                      <button onClick={handleUpdateFromModal} className="flex-1 bg-gradient-to-r from-rose-500 to-teal-500 hover:opacity-95 text-white py-2 px-3 rounded-xl font-black text-xs shadow-md shadow-rose-500/25">Update</button>  
                      <button onClick={handleDeleteFromModal} className={`py-2 px-3 rounded-xl font-black text-xs border ${darkMode ? 'bg-slate-900 border-slate-800 text-rose-400 hover:bg-rose-950/30' : 'bg-rose-50 border-rose-200 text-rose-600 hover:bg-rose-100'}`}>Delete</button>  
                    </>
                  ) : (
                    <button onClick={handleSaveFromModal} className="w-full bg-gradient-to-r from-emerald-500 to-teal-500 hover:opacity-95 text-white py-2 px-3 rounded-xl font-black text-xs shadow-md shadow-emerald-500/25">Save Patient</button>  
                  )}
                </div>
              </div>
            </div>
          ...
          {/* DASHBOARD TAB VIEW */}
          {activeTab === 'dashboard' && (
            <div className="flex-1 flex flex-col lg:flex-row gap-3 overflow-y-auto lg:overflow-hidden" ref={dropdownRef}>  
              
              <div 
                className={`flex-1 border rounded-3xl p-3 sm:p-4 flex flex-col shadow-lg overflow-y-auto lg:overflow-hidden justify-between ${  
                  darkMode ? 'bg-[#181e22]/90 border-slate-800' : 'bg-white/80 border-rose-100/60 shadow-rose-500/5 backdrop-blur-md'  
                }`}
                onClick={(e) => e.stopPropagation()}  
              >
                <div className="flex justify-between items-center pb-2 border-b border-rose-100/60 flex-shrink-0">
                  <h3 className="font-black text-xs flex items-center gap-1.5">
                    <Sparkles size={14} className="text-amber-500 animate-spin" /> Patient Form  
                  </h3>
                  <div className="flex items-center gap-1.5">
                    <button onClick={() => { setModalPatient({ Id: null, Name: '', Address: '', Amount: '', Medicine: '', Syrup: '', Additional: '', Date: getTodayDateInput(), isCrushed: false }); setIsModalOpen(true); }} className="bg-gradient-to-r from-emerald-500 to-teal-500 text-white px-2.5 py-1 rounded-xl text-[11px] font-extrabold flex items-center gap-1 shadow-md shadow-emerald-500/20">
                      <Plus size={12} /> Add Patient  
                    </button>
                    <button onClick={handleClearDashboardForm} className={`p-1.5 rounded-xl border transition ${darkMode ? 'bg-slate-900 border-slate-800 text-slate-300 hover:bg-slate-800' : 'bg-rose-50/50 border-rose-200 text-slate-700 hover:bg-rose-100'}`}>
                      <RotateCcw size={12}/>  
                    </button>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-2 my-auto py-1">
                  ...
                  <div className={`grid grid-cols-1 sm:grid-cols-3 gap-2 p-2.5 border rounded-2xl ${darkMode ? 'bg-slate-900/40 border-slate-800' : 'bg-rose-50/20 border-rose-100'}`}>
                    {dropdownData.map((drop, idx) => {  
                      ...
                      return (
                        <div key={idx} className="flex flex-col gap-0.5 relative">
                          <label className="text-[10px] font-extrabold text-rose-500 truncate">{drop.name}</label>
                          <div onClick={(e) => { e.stopPropagation(); setOpenDropdown(isOpen ? null : drop.name); }} className={`px-2.5 py-1.5 rounded-xl border text-[11px] font-bold flex justify-between items-center cursor-pointer transition h-9 ${darkMode ? 'bg-slate-900 border-slate-800 text-slate-200' : 'bg-white border-rose-100 text-slate-800 hover:border-rose-300'}`}>
                            <span className="truncate">{selectedItems.length > 0 ? selectedItems.join(", ") : `Select...`}</span>
                            <ChevronDown size={13} className="text-rose-500 flex-shrink-0" />  
                          </div>
                          {isOpen && (
                            <div className={`absolute bottom-[calc(100%+2px)] left-0 right-0 mb-1 border rounded-xl shadow-2xl z-30 max-h-32 overflow-y-auto p-1 space-y-1 ${darkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-rose-100'}`}>
                              {drop.options.map((opt, optIdx) => {
                                const isSelected = selectedItems.includes(opt);
                                return (
                                  <div 
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 items-center">
                    <label 
                      className={`flex items-center gap-2.5 px-2.5 rounded-xl font-bold text-xs cursor-pointer border h-9 transition select-none ${  
                    ...
                    >
                      <div className={`w-3.5 h-3.5 rounded-md border flex items-center justify-center transition-colors ${newPatient.isCrushed ? 'bg-emerald-500 border-emerald-500 text-white' : darkMode ? 'border-slate-700 bg-slate-800' : 'border-rose-200 bg-white'}`}>
                        {newPatient.isCrushed && <span className="text-[9px] font-black leading-none">✓</span>}  
                      </div>
                      <span className="font-extrabold text-xs">Crush Med</span>
                    </label>
                    <input placeholder="Amount" className={`px-2.5 py-1.5 rounded-xl border font-black text-xs h-9 outline-none transition ${darkMode ? 'bg-slate-900 border-slate-800 text-white focus:border-rose-500' : 'bg-rose-50/30 border-rose-100 focus:border-rose-400'}`} value={newPatient.Amount} onChange={(e) => setNewPatient({...newPatient, Amount: e.target.value})} />  
                  </div>
                </div>

                <div className="pt-2 border-t flex gap-2 flex-shrink-0 border-rose-100/60 bg-transparent">
                  <button onClick={handleSaveFromDashboard} className="flex-1 bg-gradient-to-r from-emerald-500 to-teal-500 hover:opacity-95 text-white py-2 px-2.5 rounded-xl font-black text-xs shadow-md shadow-emerald-500/25 transition h-9 flex items-center justify-center">
                    Save
                  </button>
                  <button onClick={handleUpdateFromDashboard} className="flex-1 bg-gradient-to-r from-rose-500 to-teal-500 hover:opacity-95 text-white py-2 px-2.5 rounded-xl font-black text-xs shadow-md shadow-rose-500/25 transition h-9 flex items-center justify-center">
                    Update
                  </button>
                  <button onClick={handleDeleteFromDashboard} className={`py-2 px-2.5 rounded-xl font-black text-xs border transition h-9 flex items-center justify-center ${darkMode ? 'bg-slate-900 border-slate-800 text-rose-400 hover:bg-rose-950/30' : 'bg-rose-50 border-rose-200 text-rose-600 hover:bg-rose-100'}`}>
                    Delete
                  </button>
                </div>
              ...
              <div className="w-full lg:w-[360px] xl:w-[400px] flex-shrink-0 flex flex-col gap-3 overflow-y-auto lg:overflow-hidden">
                
                <div className={`border rounded-3xl p-3.5 flex flex-col gap-2.5 shadow-lg flex-shrink-0 ${  
                  darkMode ? 'bg-[#181e22]/90 border-slate-800' : 'bg-white/80 border-rose-100/60 shadow-rose-500/5 backdrop-blur-md'  
                }`}>
                  <div className="flex justify-between items-center border-b pb-1.5 border-rose-200/50 dark:border-slate-800">
                    <h3 className="font-black text-xs sm:text-sm flex items-center gap-1.5 text-rose-600 dark:text-rose-400">
                      <HeartPulse size={15} className="text-rose-500 animate-pulse"/> Profile Summary  
                    </h3>
                    <div className="flex items-center gap-1.5">
                      <button 
                        onClick={() => setIsSlipModalOpen(true)}
                        className="bg-gradient-to-r from-rose-500 to-teal-500 text-white px-2.5 py-1 rounded-xl text-[11px] font-black flex items-center gap-1 shadow-md shadow-rose-500/20 hover:opacity-95 transition"
                      >
                        <Download size={12} /> Download Slip
                      </button>
                      <span className="text-[10px] font-extrabold px-2 py-0.5 rounded-lg bg-emerald-500/10 text-emerald-500 border border-emerald-500/20">Active</span>  
                    </div>
                  </div>

                  <div className={`p-3 rounded-2xl border flex flex-col gap-2 ${darkMode ? 'bg-slate-950/40 border-slate-800' : 'bg-rose-50/40 border-rose-200/60'}`}>
                    <div className="flex justify-between items-center">
                      <h4 className="font-black text-sm break-words pr-2">{selectedPatient ? selectedPatient.Name : (newPatient.Name || "No Patient Selected")}</h4>  
                      <span className="text-[11px] font-bold text-slate-400 whitespace-nowrap">{formatDateToDisplay(selectedPatient ? selectedPatient.Date : newPatient.Date)}</span>  
                    </div>
                    
                    {(selectedPatient?.Address || newPatient.Address) && (
  <div className="text-[11px] font-black whitespace-pre-wrap break-words">
  ...
  <div className="text-[11px] font-black whitespace-pre-wrap break-words">
    ...
    <span className="text-blue-600 dark:text-blue-400">
      {selectedPatient ? selectedPatient.Additional : newPatient.Additional}
                    ...
                    <div className="text-[11px] font-bold text-rose-600 dark:text-rose-400 leading-relaxed whitespace-pre-wrap break-words">
                      <span className="font-extrabold text-slate-400">Med: </span>  
                      {selectedPatient ? selectedPatient.Medicine : newPatient.Medicine || "-"}  
                    ...
                    <div className="flex justify-between items-center pt-2 border-t border-rose-200/40 dark:border-slate-800">
                      <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider">Amount</span>
                      <span className="text-sm font-black text-rose-500">{selectedPatient ? selectedPatient.Amount : newPatient.Amount || "0"}</span>  
                    </div>
                  </div>
                </div>

                <div className={`border rounded-3xl p-3.5 flex-1 flex flex-col overflow-hidden shadow-lg min-h-[150px] ${  
                  darkMode ? 'bg-[#181e22]/90 border-slate-800' : 'bg-white/80 border-rose-100/60 shadow-rose-500/5 backdrop-blur-md'  
                }`}>
                  <div className="flex justify-between items-center pb-2 mb-2 border-b border-rose-100/60 flex-shrink-0">
                    <h3 className="font-black text-xs flex items-center gap-1.5">
                      <Users size={14} className="text-teal-500" /> Recent Patients  
                    </h3>
                    <span className="text-[10px] font-extrabold px-2 py-0.5 rounded-lg bg-rose-500/10 text-rose-500 border border-rose-500/20">{patients.length}</span>  
                  </div>
                  ...
                  <div className="flex gap-2 mb-2 flex-shrink-0">
                    <div className="relative flex-1">
                      ...
                      <input 
                        ...
                        className={`w-full pl-8 pr-3 py-2 rounded-xl border text-xs font-semibold outline-none transition h-9 ${  
                    ...
                    <button 
                      ...
                      title="Search"
                      className={`px-2.5 py-2 rounded-xl border text-xs font-bold transition h-9 flex items-center justify-center gap-1 ${  
                    ...
                    {searchTerm && (
                      <button 
                        ...
                        title="Clear Search"
                        className={`px-3 py-2 rounded-xl border text-xs font-bold transition h-9 flex items-center justify-center ${  
                      ...
                      >
                        <div className="flex flex-col gap-0.5 truncate pr-2">
                          <div className="flex items-center gap-2">
                            <span className="font-black text-xs truncate">{pat.Name}</span>  
                            {pat.Address && (
                              <span className="text-[11px] text-slate-400 truncate max-w-[110px]">({pat.Address})</span>  
                            )}
                          </div>
                          <span className="text-[11px] text-teal-600 dark:text-teal-400 font-bold truncate">{pat.Medicine || pat.Syrup || "-"}</span>  
                        </div>
                        <div className="flex flex-col items-end gap-0.5 flex-shrink-0">
                          <span className="text-xs font-black text-rose-500">{pat.Amount || "0"}</span>  
          ...
          {activeTab === 'all-patients' && (
            <div className="flex-1 flex gap-3 overflow-y-auto lg:overflow-hidden">
              <div className={`flex-1 border rounded-3xl p-3.5 sm:p-4 flex flex-col overflow-hidden shadow-lg ${  
                darkMode ? 'bg-[#181e22]/90 border-slate-800' : 'bg-white/80 border-rose-100/60 shadow-rose-500/5 backdrop-blur-md'  
              }`}>
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2.5 pb-2.5 mb-3 border-b border-rose-100/60 flex-shrink-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <h3 className="font-black text-xs sm:text-sm flex items-center gap-1.5">
                      <Users size={16} className="text-teal-500 animate-bounce" /> All Patients Directory  
                    </h3>
                    <button 
                      onClick={() => { 
                      ...
                      className="bg-gradient-to-r from-emerald-500 to-teal-500 text-white px-3 py-1.5 rounded-xl text-xs font-extrabold flex items-center gap-1 shadow-md shadow-emerald-500/20"
                    >
                  ...
                  <table className="w-full min-w-[700px] text-left border-collapse text-xs">
                    <thead className={`sticky top-0 z-10 border-b ${darkMode ? 'bg-slate-900 border-slate-800 text-slate-300' : 'bg-rose-50/90 border-rose-100 text-slate-700'}`}>
                      <tr>
                        ...
                        <th className="p-3 font-black">Address</th>
                        <th className="p-3 font-black">Medicine</th>
                        <th className="p-3 font-black">Syrup</th>
                        <th className="p-3 font-black">Additional</th>
                    ...
                    <tbody className="divide-y divide-rose-100/40 dark:divide-slate-800 font-semibold">
                      {patients.map((pat, idx) => (
                        ...
                        >
                          <td className="p-3 font-black whitespace-pre-wrap break-words">{pat.Name}</td>  
                          <td className="p-3 text-slate-400 whitespace-nowrap font-bold">{formatDateToDisplay(pat.Date)}</td>  
                          <td className="p-3 text-slate-700 dark:text-slate-200 whitespace-pre-wrap break-words">{pat.Address || "-"}</td>  
                          <td className="p-3 text-teal-600 dark:text-teal-400 font-bold whitespace-pre-wrap break-words">{pat.Medicine || "-"}</td>  
                          <td className="p-3 text-indigo-600 dark:text-indigo-400 font-bold whitespace-pre-wrap break-words">{pat.Syrup || "-"}</td>  
                          <td className="p-3 text-slate-700 dark:text-slate-200 whitespace-pre-wrap break-words">{pat.Additional || "-"}</td>  
                          <td className="p-3 font-black text-rose-500 whitespace-nowrap">{pat.Amount || "0"}</td>  
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          {/* ANALYTICS TAB VIEW */}
          {activeTab === 'analytics' && (
            <div className={`flex-1 border rounded-3xl p-4 sm:p-5 flex flex-col gap-4 overflow-y-auto shadow-lg ${  
              darkMode ? 'bg-[#181e22]/90 border-slate-800' : 'bg-white/80 border-rose-100/60 shadow-rose-500/5 backdrop-blur-md'  
            }`}>
              <div className="flex items-center justify-between pb-3 border-b border-rose-100/60">
                <div className="flex items-center gap-2.5">
                  ...
                  <h3 className="font-black text-sm sm:text-base">Clinic Analytics & Performance Graphs</h3>  
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                <div className={`p-3.5 rounded-2xl border flex flex-col gap-2 ${darkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-rose-50/30 border-rose-100'}`}>
                  <div className="flex items-center justify-between text-slate-400">
                    <span className="text-xs font-extrabold">Total Patients</span>
                    <Users size={16} className="text-teal-500" />  
                  </div>
                  <span className="text-2xl font-black text-teal-600 dark:text-teal-400">{totalPatientsCount}</span>  
                  <span className="text-[10px] font-semibold text-slate-400">Synced from Database</span>
                </div>

                <div className={`p-3.5 rounded-2xl border flex flex-col gap-2 ${darkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-rose-50/30 border-rose-100'}`}>
                  <div className="flex items-center justify-between text-slate-400">
                    <span className="text-xs font-extrabold">Total Revenue</span>
                    <DollarSign size={16} className="text-rose-500" />  
                  </div>
                  <span className="text-2xl font-black text-rose-500">Rs. {totalRevenue.toLocaleString()}</span>  
                  <span className="text-[10px] font-semibold text-slate-400">Overall collection amount</span>
                </div>

                <div className={`p-3.5 rounded-2xl border flex flex-col gap-2 ${darkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-rose-50/30 border-rose-100'}`}>
                  <div className="flex items-center justify-between text-slate-400">
                    <span className="text-xs font-extrabold">Today's Entries</span>
                    <Activity size={16} className="text-emerald-500" />  
                  </div>
                  <span className="text-2xl font-black text-emerald-600 dark:text-emerald-400">{todayPatientsCount}</span>  
                    <span className="text-[10px] font-semibold text-slate-400">Added on current date</span>
                </div>

                <div className={`p-3.5 rounded-2xl border flex flex-col gap-2 ${darkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-rose-50/30 border-rose-100'}`}>
                  <div className="flex items-center justify-between text-slate-400">
                    <span className="text-xs font-extrabold">Avg. Per Patient</span>
                    <TrendingUp size={16} className="text-indigo-500" />  
                  </div>
                  <span className="text-2xl font-black text-indigo-600 dark:text-indigo-400">Rs. {avgRevenuePerPatient.toLocaleString()}</span>  
                  <span className="text-[10px] font-semibold text-slate-400">Average ticket size</span>
                </div>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                
                <div className={`p-4 rounded-3xl border flex flex-col gap-3 ${darkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-rose-50/20 border-rose-100'}`}>
                  <div className="flex items-center justify-between">
                    <h4 className="font-black text-xs sm:text-sm flex items-center gap-1.5">
                      <BarChart3 size={16} className="text-rose-500" /> Revenue & Patient Overview  
                    </h4>
                    <span className="text-[10px] font-bold text-slate-400">Metric Representation</span>
                  </div>
                  ...
                  <div className="space-y-2.5 pt-1">
                    <div>
                      ...
                      <div className={`w-full h-2.5 rounded-full overflow-hidden ${darkMode ? 'bg-slate-800' : 'bg-rose-100'}`}>
                        <div 
                  ...
                  <div className="pt-1 text-[11px] font-semibold text-slate-400 leading-relaxed">
                    * Metrics are automatically calculated from all {totalPatientsCount} entries stored in your Supabase database table.  
                  </div>
                </div>

                <div className={`p-4 rounded-3xl border flex flex-col justify-between ${darkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-rose-50/20 border-rose-100'}`}>
                  <div className="space-y-2">
                    <h4 className="font-black text-xs sm:text-sm flex items-center gap-1.5">
                      <ShieldCheck size={16} className="text-indigo-500" /> Database & System Health  
                    </h4>
                  ...
                  <div className={`p-3 rounded-2xl border flex items-center justify-between ${darkMode ? 'bg-slate-950/60 border-slate-800' : 'bg-white/80 border-rose-100'}`}>
                    <span className="text-xs font-bold">Real-time Data Sync</span>
                    <span className="text-[11px] font-black text-emerald-500 bg-emerald-500/10 px-2.5 py-0.5 rounded-lg border border-emerald-500/20">Operational</span>
                  </div>
                </div>
          ...
          {activeTab === 'settings' && (
            <div className={`flex-1 border rounded-3xl p-4 sm:p-5 flex flex-col gap-4 overflow-y-auto shadow-lg ${  
              ...
              <div className="flex items-center gap-2.5 pb-3 border-b border-rose-100/60">
                <SettingsIcon size={20} className="text-rose-500" />  
                <h3 className="font-black text-sm sm:text-base">Application Settings</h3>  
              </div>

              <div className="space-y-3 max-w-xl">
                <div className={`p-4 rounded-2xl border flex items-center justify-between ${darkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-rose-50/30 border-rose-100'}`}>
                  <div>
                    <h4 className="font-black text-xs sm:text-sm">Appearance Mode</h4>
                    <p className="text-[11px] text-slate-400 font-semibold">Switch between dark and light themes</p>
                  </div>
                  <button 
                    onClick={() => setDarkMode(!darkMode)}
                    className="px-3.5 py-2 rounded-xl bg-gradient-to-r from-rose-500 to-teal-500 text-white font-bold text-xs"
                  >
                    Toggle Theme
                  </button>
                </div>
